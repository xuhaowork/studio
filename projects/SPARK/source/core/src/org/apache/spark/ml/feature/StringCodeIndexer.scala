...
class StringCodeIndexer extends Serializable {
  ...
  def fit(dataset: DataFrame): StringCodeIndexerModel = {
    Tools.columnExists(inputCol, dataset, true)
    import org.apache.spark.sql.columnUtils.DataTypeImpl._
    val dt = dataset.schema(inputCol).dataType
    require(dataset.schema(inputCol).dataType in "string",
      s"'$inputCol'需要是string类型，而现在是'${dt.simpleString}类型'。")
    Tools.validNewColName(outputCol, dataset)
