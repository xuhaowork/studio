...
import org.apache.spark.mllib.linalg.{VectorUDT => mllibVectorUDT, Vector => mllibVector}
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val attrs = $(inputCols).flatMap { c =>
      ...
      field.dataType match {
        ...
        case _: NumericType | BooleanType =>
          ...
          Some(NumericAttribute.defaultAttr.withName(c))
          // 处理 mllib.VectorUDT的metadata
        case _: mllibVectorUDT =>
          // @todo: 这是spark留下的一个bug，2.0以后sql中涉及到VectorUDT指的是ml.linalg.VectorUDT而不是
          // @todo: mllib.linalg.VectorUDT，暂时不取出来而是以默认attribute应对
    ...
    val args = $(inputCols).map { c =>
      schema(c).dataType match {
        ...
        case _: mllibVectorUDT => dataset(c)
